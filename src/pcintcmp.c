    ...
    case 'C':
        ...
        do
        {
            ...
            case 'R':
                ...
                *((double *)(gs->rconst0 + pcode->q)) = rvalue;
         ...
         pcode++)
    {
        ...
        if (memcmp(gs->ot[pcode->op].opcode, "ENT", 3) == 0)
        {
            ...
            for (pcode2 = pcode + 1;
                 ...
                 pcode2++)
            {
                if (memcmp(gs->ot[pcode2->op].opcode, "ENT", 3) == 0)
                    break;

                if (memcmp(gs->ot[pcode2->op].opcode, "DEF", 3) != 0)
                    continue;
...

{
    ...
    for (cp = cpstart; cp < cpstop; cp += 16)
    {
        ...
        {
            if (cp + i >= cpx && cp + i < cpx + len)
                if (strchr(zulzeichen, *(cp + i)) != NULL && *(cp + i) != '\0')
